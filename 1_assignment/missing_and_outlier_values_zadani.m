...
color = [repmat([0,0,0],100,1);  repmat([1,0,0],100,1)];
...
m_v = repmat(m, 200,1);
%
standardized_knndata_pca = (knndata_pca - repmat(mean(knndata_pca), length(knndata_pca), 1)) ./ repmat(std(knndata_pca), length(knndata_pca), 1);
...
while(1)    
    ...
    [newP, newC, sumD, D] = kmeans(X,m,'replicates',nReplicates);     
    ...
    for j = 1:size(cs,1)
        ...
        if numSamples > 1
            all_dists = D(:,j);
            [s_max, ind_max] = max(all_dists);
            [s_min, ind_min] = min(all_dists);
            distortion = s_min/s_max;
            if distortion < th
               newX(ind_max,:) = [];
               c_coords = newC(j,:);
               c_new_coords = (c_coords * numSamples - s_max)/(numSamples - 1);
               newC(j,:) = c_new_coords;
               newP(ind_max) = [];
               D(ind_max,:) = [];
            end
